...
class TestEmergencyCharging(unittest.TestCase):
    ...
    def test_emergency_charging_lifecycle(self):
        ...
        runtime['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=180)).isoformat()
        v = values(runtime)
        self.assertEqual(v['low_power_duration_s'], 180)
        self.assertFalse(v['emergency_charging'])

        # 3. Exactly 5 minutes (300s) -> emergency_charging turns True!
        runtime['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=300)).isoformat()
        v = values(runtime)
        self.assertEqual(v['low_power_duration_s'], 300)
        self.assertTrue(v['emergency_charging'])

        # 4. 6 minutes (360s) -> remains True
        runtime['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=360)).isoformat()
        v = values(runtime)
        ...
        runtime['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=370)).isoformat()
        v = values(runtime)
        self.assertTrue(v['emergency_charging'])

        # 7. Connector reseated properly, charging speed jumps to 7.0 kW -> resets!
        runtime['latest']['data']['charge_power_w'] = 7000
        runtime['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=400)).isoformat()
        v = values(runtime)
    ...
    def test_threshold_boundary(self):
        ...
        values(runtime2)
        runtime2['latest']['data']['measured_at'] = (self.base_time + timedelta(seconds=300)).isoformat()
        v2 = values(runtime2)
