  ...
  overview(v){
    ...
    const quickMetrics=charging
      ?`${metric('총 주행거리',n(v.odometer_km,0),'km','counter')}`+
       ...
       `${metric('충전 전력 · 추정',n(powerKw,1),'kW','ev-station','충전 중')}`+
    ...
    return `<div class="cockpit"><section class="hero"><div class="hero-copy"><h2>${esc(status).replace('\n','<br>')}</h2></div>${this.vehicleImage()}</section><div class="quick"><section class="energy ${charging?'is-charging':''}" style="--soc:${soc??0}%">${sweepHtml}${markersHtml}<div class="energy-head"><div class="battery-label"><span>${charging?'충전중':'배터리 잔량'}</span></div><strong class="soc-value">${n(soc,0)}<small>%</small></strong></div></section><div class="quick-metrics">${quickMetrics}</div></div></div><div class="overview-links"><button class="shortcut" data-tab="parking"><span><b>주차 위치</b><small>${v.parking_latitude==null?'위치 수신 대기':time(v.parking_at)}</small></span><em>지도 →</em><div class="mini-map parking-mini"></div></button><button class="shortcut" data-tab="trips"><span><b>최근 주행</b><small>${latest?n(latest.distance_m==null?null:latest.distance_m/1000,2)+' km':'기록 없음'}</small><small>${latest?shortDuration(latest.duration_s):'새 주행 기록을 기다립니다'}</small></span><em>보기 →</em><div class="mini-map trip-mini"></div></button></div><div class="mini-condition"><span>외기 <b>${n(v.outside_temp_c)}°C</b></span><span>12V <b>${n(v.aux_voltage,1)}V</b></span><span>공조 <b>${v.ac_on==null?'—':v.ac_on?'ON':'OFF'}</b></span></div>`;
  }
  vehicleImage(){
