...
`;
  ...
  tripHistory(isTrip){
    ...
    const labels=d=>d.date.getUTCDate()+'일('+['일','월','화','수','목','금','토'][d.date.getUTCDay()]+')';
    return `<section class="panel trip-history"><div class="paneltitle"><h2>최근 주행</h2><span class="sub">최근 7일</span></div><div class="trip-days">${days.map(d=>`<button class="trip-day" data-trip-day="${d.key}" aria-pressed="${d.key===this.tripDay}" aria-label="${d.key}, ${d.indices.length} 회 주행"><span class="trip-today">${d.today?'오늘':'&nbsp;'}</span><b>${labels(d)}</b><span class="trip-count">${icon('road')}${d.indices.length}</span></button>`).join('')}</div>${selected?`<div class="trip-day-heading">${selected.key} · ${selected.indices.length} 회 주행</div><div class="scroll">${selected.indices.length?selected.indices.map(i=>{const e=this.trips[i];const ed=e?.data||{};const durText=tripDurationKo(ed.duration_s);const hasSoc=ed.start_soc_percent!=null&&ed.end_soc_percent!=null;const effHtml=ed.efficiency_km_kwh!=null?`<span class="trip-eff">${n(ed.efficiency_km_kwh,1)} km/kWh</span>`:'';const socHtml=hasSoc?`<span class="trip-soc">${icon(batteryIconName(ed.start_soc_percent))} ${Math.round(ed.start_soc_percent)}% → ${icon(batteryIconName(ed.end_soc_percent))} ${Math.round(ed.end_soc_percent)}%${effHtml}</span>`:`<small>${duration(ed.duration_s)}</small>`;return `<button class="tripbtn ${isTrip&&i===this.selected?'selected':''}" data-trip="${i}"><span><b>${timeOnly(ed.started_at||e.observed_at)}${durText?`<span class="trip-dur">${durText}</span>`:''}</b>${socHtml}</span><strong>${n((ed.distance_m||0)/1000,2)} <small>km</small></strong></button>`;}).join(''):'<div class="empty">기록된 주행이 없습니다.</div>'}</div>`:'<div class="empty">날짜를 선택하면 해당 날짜의 주행 기록이 표시됩니다.</div>'}</section>`;
  }
  overview(v){
