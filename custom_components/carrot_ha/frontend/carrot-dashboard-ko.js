  ...
  tripHistory(isTrip){
    ...
    const daysHtml = days.map(d => `
      <button class="trip-day ${d.key === this.tripDay ? 'active' : ''}" 
              ...
              aria-label="${d.key}, ${d.indices.length}회 주행">
    ...
    if (visibleIndices.length === 0) {
      cardsHtml = `<div class="empty" style="grid-column: 1 / -1; padding: 40px 10px;">기록된 주행이 없습니다.</div>`;
    } else {
    ...
    const paginationHtml = `
      <div class="panel-pagination">
        ...
        <span class="page-indicator-text">
          ${this._tripPage} / ${totalPages} 페이지 (총 ${totalItems}개)
        </span>
    ...
    const mergeSubText = isMergeActive
      ? (hasMergedTrips ? '(30분 이하 인접 주행 병합됨)' : '')
      : '(개별 분할 표시)';
    ...
    return `
      <section class="panel trip-history">
          ...
          <div class="day-timeline-wrap">
            <div class="day-timeline-topline">
              <span class="day-timeline-title">
                <strong>${selected.key} · ${dayIndices.length}회 주행</strong>
                <small class="day-timeline-merge-sub">${mergeSubText}</small>
              </span>
              <button class="merge-toggle-badge ${isMergeActive ? '' : 'off'}" id="btnToggleTripMerge" title="클릭하여 30분 이하 인접 주행 병합 토글">
                ${isMergeActive ? '30분 이하 병합됨' : '개별 분할 표시'}
              </button>
            </div>
        ...
        ` : `
          <div class="empty">날짜를 선택하면 해당 날짜의 주행 기록이 표시됩니다.</div>
        `}
      </section>
    `;
  }
