...
`;
  ...
  body(v,trip,isTrip){
    ...
    if(this.tab==='charge')return `${this.batteryHistory()}<div class="tiles">${metric('이번 달 충전량',n(v.month_charge_kwh),'kWh','battery-plus')}${metric('이번 달 충전 요금',n(v.month_charge_cost,0),'원','cash','추정치')}${metric('완속 분류',n(v.month_slow_kwh),'kWh','power-plug')}${metric('급속 분류',n(v.month_fast_kwh),'kWh','flash')}</div><div class="panel"><div class="paneltitle"><h2>충전 기록</h2><span class="sub">최근 ${this.charges.length}개</span></div>${this.charges.length?this.charges.map(e=>`<div class="row"><span>${time(e.data.started_at)}<br>${duration(e.data.duration_s)}</span><b>${n(e.data.energy_kwh,2)} kWh<br><span>${e.data.partial?'일부 구간만 수집':'기록된 충전량'}</span></b></div>`).join(''):'<div class="empty">충전 기록을 기다리고 있습니다.<br>수집기가 켜져 있는 동안 충전 기록이 저장됩니다.</div>'}</div><p class="notice">배터리 에너지 증가로 추정합니다. 11kW 이하를 완속으로 분류하며, 요금 ${n(v.month_charge_cost,0)}원은 기본 단가 기준 추정입니다. 차량이 잠들거나 콤마 전원이 꺼지면 측정할 수 없습니다.</p>`;
    ...
    return `<div class="tiles">${tiles}</div><div class="layout"><section class="panel"><div class="paneltitle"><h2>${isTrip?'주행 상세':'주차 위치'}</h2><span class="sub">${time(isTrip?trip.started_at:v.parking_at)}</span></div><div class="map"></div><div class="route-caption">${isTrip?`<div class="legend"><span>저속 · 0 km/h</span><i class="gradient"></i><span>고속 · ${n(this.maxSpeed(trip.route),0)} km/h</span></div><div class="sub">${time(trip.started_at)} → ${time(trip.ended_at)}<br>${(trip.route||[]).length}개 경로 좌표 · 출발 하늘색 / 도착 파랑</div>`:`<b>${v.parking_latitude!=null?`${n(v.parking_latitude,5)}, ${n(v.parking_longitude,5)}`:'위치 정보 대기 중'}</b><div class="sub">최근 주차 또는 마지막 주행 도착 위치</div>`}</div></section><section class="panel"><div class="paneltitle"><h2>최근 주행</h2><span class="sub">전체 ${n(v.trip_count,0)}개</span></div><div class="scroll">${this.trips.length?this.trips.map((e,i)=>`<button class="tripbtn ${isTrip&&i===this.selected?'selected':''}" data-trip="${i}"><span><b>${time(e.data.started_at||e.observed_at)}</b><small>${duration(e.data.duration_s)}</small></span><strong>${n((e.data.distance_m||0)/1000,2)} <small>km</small></strong></button>`).join(''):'<div class="empty">저장된 주행이 없습니다.</div>'}</div><div class="pages"><button class="page" data-page="-1" ${this.offset===0?'disabled':''}>이전</button><span class="sub">${this.offset/20+1} 페이지</span><button class="page" data-page="1" ${this.trips.length<20?'disabled':''}>다음</button></div></section></div>${!isTrip?`<h2 class="section">차량 컨디션</h2><div class="tiles">${metric('외부 온도',n(v.outside_temp_c),'°C','thermometer')}${metric('12V 배터리',n(v.aux_voltage,2),'V','car-battery')}${metric('에어컨',v.ac_on==null?'—':v.ac_on?'ON':'OFF','','snowflake')}${metric('송풍 단계',n(v.blower_level,0),'','fan')}</div>`:''}`;
  }
  overview(v){
