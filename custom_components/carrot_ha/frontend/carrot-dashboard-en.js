  ...
  body(v,trip,isTrip){
    ...
    if(this.tab==='parking'){
      ...
      const tiles=`
        ...
        ${isDriving?metric('Current speed',n(v.wheel_speed_kph??v.speed_kph,0),'km/h','speedometer','Live cluster speed'):metric('12V battery',n(v.aux_voltage,1),'V','car-battery',auxStatus)}
    ...
    const routeCaption=isTrip
      ?(isSpecificTrip
          ...
          :`<div class="sub">${dayTrips.length?'Showing all trip routes for selected date · Day start (sky blue) / Day arrival (blue) · Select an individual trip on the right for details':'No trips recorded for this date'}</div>`
        )
  ...
  overview(v){
    ...
    const quickMetrics = charging
      ...
      : `${renderLockMetric(isLocked, false, openDoors)}` +
        ...
        `${metric('Charged this month', n(v.month_charge_kwh), 'kWh', 'battery-plus')}` +
        `${metric('Charge cost this month', n(v.month_charge_cost, 0), 'KRW', 'cash', '(est.)')}`;
  ...
  async drawMap(route,v,dayTrips=[]){
    ...
    try{
      ...
      if(points.length){
        ...
        const coords=points.map(p=>[p.latitude,p.longitude]);bounds=L.latLngBounds(coords);this.map.fitBounds(bounds,{padding:[32,32],maxZoom:16});marker(coords[0],false,'S','Start');marker(coords.at(-1),true,'E','End');
        ...
        if(allDayCoords.length&&validTrips.length){
          ...
          marker([startPt.latitude,startPt.longitude],false,'S','Day start');
          marker([endPt.latitude,endPt.longitude],true,'E','Day arrival');
        }else if(targetPos){
