...
const tripDurationEn = s => {
  ...
  if(totalSec < 60) return 'Drove < 1 min';
  const h = Math.floor(totalSec / 3600), m = Math.floor((totalSec % 3600) / 60);
  if(h > 0 && m > 0) return `Drove for ${h}h ${m}m`;
  if(h > 0) return `Drove for ${h}h`;
  return `Drove for ${m}m`;
};
...
`;
  ...
  tripHistory(isTrip){
    ...
    const labels=d=>d.date.toLocaleDateString('en-US',{day:'numeric',weekday:'short',timeZone:'UTC'});
    return `<section class="panel trip-history"><div class="paneltitle"><h2>Recent trips</h2><span class="sub">Last 7 days</span></div><div class="trip-days">${days.map(d=>`<button class="trip-day" data-trip-day="${d.key}" aria-pressed="${d.key===this.tripDay}" aria-label="${d.key}, ${d.indices.length} trips"><span class="trip-today">${d.today?'Today':'&nbsp;'}</span><b>${labels(d)}</b><span class="trip-count">${icon('road')}${d.indices.length}</span></button>`).join('')}</div>${selected?`<div class="trip-day-heading">${selected.key} · ${selected.indices.length} trips</div><div class="scroll">${selected.indices.length?selected.indices.map(i=>{const e=this.trips[i];const ed=e?.data||{};const durText=tripDurationEn(ed.duration_s);const hasSoc=ed.start_soc_percent!=null&&ed.end_soc_percent!=null;const socHtml=hasSoc?`<span class="trip-soc">${icon(batteryIconName(ed.start_soc_percent))} ${Math.round(ed.start_soc_percent)}% → ${icon(batteryIconName(ed.end_soc_percent))} ${Math.round(ed.end_soc_percent)}%</span>`:`<small>${duration(ed.duration_s)}</small>`;return `<button class="tripbtn ${isTrip&&i===this.selected?'selected':''}" data-trip="${i}"><span><b>${timeOnly(ed.started_at||e.observed_at)}${durText?`<span class="trip-dur">${durText}</span>`:''}</b>${socHtml}</span><strong>${n((ed.distance_m||0)/1000,2)} <small>km</small></strong></button>`;}).join(''):'<div class="empty">No trips recorded.</div>'}</div>`:'<div class="empty">Choose a date to see its trips.</div>'}</section>`;
  }
  overview(v){
