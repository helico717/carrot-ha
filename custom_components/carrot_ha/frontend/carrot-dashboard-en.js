...
`;
  ...
  body(v,trip,isTrip){
    ...
    const tiles=isTrip?`${metric('Distance',n(trip.distance_m==null?null:trip.distance_m/1000,2),'km','map-marker-distance')}${metric('Duration',duration(trip.duration_s),'','timer-outline')}${metric('Average speed',n(trip.duration_s?trip.distance_m/trip.duration_s*3.6:null,0),'km/h','speedometer-medium')}${metric('Top speed',n(this.maxSpeed(trip.route),0),'km/h','speedometer')}${metric('Efficiency',n(trip.efficiency_km_kwh,1),'km/kWh','lightning-bolt',trip.energy_wh!=null?n(trip.energy_wh/1000,2)+' kWh used':'')}${metric('SOC used',n(trip.soc_used_percent,1),'%','battery-minus',trip.start_battery_wh!=null?n(trip.start_battery_wh/1000,1)+' → '+n(trip.end_battery_wh/1000,1)+' kWh':'')}`:
  ...
  vehicleStatus(v){
    ...
    if(stale) {
      const last=driving===true?'driving':v.charging===true?'charging':driving===false?'parked':null;
      const measured=measuredAge===null?'measurement time unavailable':`measured ${Math.floor(measuredAge/60)} min ago`;
      return {key:'stale',label:last?`Last known: ${last} · ${measured}`:`Current state unknown · ${measured}`};
    }
