// === Inlined panda_counter_health helper functions ===
...
function attachPandaCounterHealth(payload, previousPayload, previousUpdatedAt, updatedAt) {
...
function buildAiPandaInterface(rawPanda) {
...
async function fetchVehicleStatus() { return null; }
...
async function handleTrips(request, env, pathname) {
  ...
  const trips = await env.DB.prepare(`
    SELECT id, device_id, started_at, ended_at, duration_s, distance_m,
           ...
           route_json
    FROM trips ORDER BY ended_at DESC, id DESC LIMIT ? OFFSET ?
  `).bind(limit, Math.max(0, Number.parseInt(url.searchParams.get("offset") || "0", 10) || 0)).all();

  return tripHistoryJson({ trips: (trips.results || []).map(t => url.searchParams.get("include_route") === "true" ? {...parseTripSummary(t), ...parseTripRoute(t)} : parseTripSummary(t)) }, "d1");
}
...
async function handleAiTrips(request, env, pathname) {
  ...
  return json({
    ...
    generatedAt: nowIso(),
    trips: (trips.results || []).map(t => url.searchParams.get("include_route") === "true" ? {...parseTripSummary(t), ...parseTripRoute(t)} : parseTripSummary(t)),
  });
}
...
async function handleRecordedTrip(request,env) {
  if (!authorize(request,env,true)) return json({error:"unauthorized"},401);
  const payload=await request.clone().json();
  const response=await handleTrip(request,env);
...
async function handleArchivedTelemetry(request, env) {
  if (!authorize(request,env,true)) return json({error:"unauthorized"},401);
  const payload=await request.clone().json();
  if (!payload.deviceId || !payload.updatedAt || !Number.isFinite(Date.parse(payload.updatedAt))) return json({error:"invalid_telemetry"},400);
  const previous=await env.DB.prepare("SELECT updated_at FROM latest_state WHERE device_id=?").bind(payload.deviceId).first();
  if (!previous || Date.parse(payload.updatedAt)>=Date.parse(previous.updated_at)) {
    const result=await handleTelemetry(request,env);
    if (result.status!==200) return result;
  }
  await env.DB.prepare("INSERT OR IGNORE INTO telemetry_history(device_id,updated_at,raw_json) VALUES (?,?,?)").bind(payload.deviceId,payload.updatedAt,JSON.stringify(payload)).run();
  return json({ok:true});
}
